...
!======================================================================!
!                                                                      !
!   PURPOSE:    Provides subroutines implementing various root-finding !
!               algorithms, as well as a global bracket finder.        !
!   CONTAINS:   bisct (subroutine), brentmul (subroutine),             !
!               globisec (subroutine), globmul (subroutine),           !
!               globrack (subroutine)                                  !
!                                                                      !
!----------------------------------------------------------------------!
MODULE rootfind
CONTAINS
    ...
    !   MODIFICATIONS:  None                                           !
    !                                                                  !
    !   PURPOSE:        A root finder using the hybrid bisection-      !
    !                   secant algorithm. Returns a computed value of  !
    !                   the root within xleft and xright once          !
    !                   error < maxerr or has run maximum number of    !
    !                   iterations.                                    !
    !                                                                  !
    ...
    !                                                                  !
    !------------------------------------------------------------------!
    SUBROUTINE biscnt   ( error   , fnctn   , maxerr  , maxsteps,      &
...
!                 ( xnext /= xnext ) ) THEN
    ...
    !                   of values. This subroutine uses the hybrid     !
    ...
    !                   to the caller.                                 !
    !                                                                  !
    !   EXTERNALS:      biscnt  , globrack                             !
    !                                                                  !
    !------------------------------------------------------------------!
    SUBROUTINE globisec ( dx      , error   , fnctn   , maxerr  ,      &
    ...
    !   MODIFICATIONS:  None                                           !
    !                                                                  !
    !   PURPOSE:        A global bracket finder. For a given function  !
    !                   it finds values on each side of each of the    !
    !                   function's roots within a given range.         !
    !                                                                  !
    !   ARGUMENTS:      brackets, a 2 by n real array in which the     !
    !                             left and right brackets will be      !
    !                             stored and returned. Will find up to ! 
    !                             n sets of brackets.                  !
    !                  *dx      , the initial size of increment to use !
    !                             when examining function. Minimum     !
    !                             interval will be 0.01 of this.       !
    !                  *fnctn   , the Fortran function for which the   !
    !                             brackets will be found. Must take    !
    !                             only one real argument, return a     !
    !                             real argument.                       !
    !                   numroots, an integer value which will return   !
    !                             the number of roots for which        !
    !                             brackets were found. A negative      !
    ...
    !                             occurred.                            !
    !                   verbose , a logical variable which specifies   !
    !                             whether to print to stdout any       !
    !                             bracket values which are found and   !
    !                             warning messages when 'dx' set to    !
    !                             'dxmin'.                             !
    !                  *xmax    , the upper limit of the range on      !
    !                             which the subroutine will search for !
    !                             roots and brackets.                  !
    ...
    !                                                                  !
    !------------------------------------------------------------------!
    SUBROUTINE globrack ( brackets, dx      , fnctn   , numroots,      &
                          verbose , xmax    , xmin     )
        IMPLICIT NONE

        ! Give compiler information about function for which to find the 
        ! brackets
        INTERFACE
