...
!               subroutines. Lines in the file beginning with the pound symbol !
...
CONTAINS
    ...
    !   MODIFICATIONS:  None                                                   !
    !                                                                          !
    !   PURPOSE:        Reads in a 1 byte integer 2D array of unknown size     !
    !                   into an allocatable array.                             !
    !                                                                          !
    !   ARGUMENTS:     *filename, the file from which to read in the array.    !
    !                       CHARACTER                                          !
    !                   i1array_dyn, the allocatable array which is to be      !
    !                       filled with data read in from the file. INTEGER(1) !
    !                   [errval], returns any error codes generated during the !
    !                       execution of this subroutine. INTEGER              !
    !                  *[clobber = 'clobber'], determines whether to overwrite !
    !                       array if already allocated. Options are 'clobber'  !
    ...
    !   MODIFICATIONS:  None                                                   !
    !                                                                          !
    !   PURPOSE:        Reads in an 8 byte integer 2D array of unknown size    !
    !                   into an allocatable array.                             !
    !                                                                          !
    !   ARGUMENTS:     *filename, the file from which to read in the array.    !
    !                       CHARACTER                                          !
    !                   i8array_dyn, the allocatable array which is to be      !
    !                       filled with data read in from the file. INTEGER(8) !
    !                   [errval], returns any error codes generated during the !
    !                       execution of this subroutine. INTEGER              !
    !                  *[clobber = 'clobber'], determines whether to overwrite !
    !                       array if already allocated. Options are 'clobber'  !
    ...
    !   MODIFICATIONS:  None                                                   !
    !                                                                          !
    !   PURPOSE:        Reads in a 16 byte 2D real array of unknown size into  !
    !                   an allocatable array.                                  !
    !                                                                          !
    !   ARGUMENTS:     *filename, the file from which to read in the array.    !
    !                       CHARACTER                                          !
    !                   r16array_dyn, the allocatable array which is to be     !
    !                       filled with data read in from the file. REAL(16)   !
    !                   [errval], returns any error codes generated during the !
    !                       execution of this subroutine. INTEGER              !
    !                  *[clobber = 'clobber'], determines whether to overwrite !
    !                       array if already allocated. Options are 'clobber'  !
    ...
    !   MODIFICATIONS:  None                                                   !
    !                                                                          !
    !   PURPOSE:        Reads in a 1 byte integer 2D array whose dimensions    !
    !                   are known in advance. Note that there is no way to     !
    !                   know if some data was missed because the array was too !
    !                   small.                                                 !
    !                                                                          !
    !   ARGUMENTS:     *filename, the file from which to read in the array.    !
    !                       CHARACTER                                          !
    !                   i1array, the array which is to be filled with values   !
    !                       read in from the file. INTEGER(1)                  !
    !                   nrows, returns the number of rows of data which were   !
    !                       read into the array. INTEGER                       !
    !                   [errval], returns any error codes generated during the !
    !                       execution of this subroutine. INTEGER              !
    ...
    !   MODIFICATIONS:  None                                                   !
    !                                                                          !
    !   PURPOSE:        Reads in an 8 byte integer 2D array whose dimensions   !
    !                   are known in advance. Note that there is no way to     !
    !                   know if some data was missed because the array was too !
    !                   small.                                                 !
    !                                                                          !
    !   ARGUMENTS:     *filename, the file from which to read in the array.    !
    !                       CHARACTER                                          !
    !                   i8array, the array which is to be filled with values   !
    !                       read in from the file. INTEGER(8)                  !
    !                   nrows, returns the number of rows of data which were   !
    !                       read into the array. INTEGER                       !
    !                   [errval], returns any error codes generated during the !
    !                       execution of this subroutine. INTEGER              !
    ...
    !   MODIFICATIONS:  None                                                   !
    !                                                                          !
    !   PURPOSE:        Reads in a quad precision 2D array whose dimensions    !
    !                   are known in advance. Note that there is no way to     !
    !                   know if some data was missed because the array was too !
    !                   small.                                                 !
    !                                                                          !
    !   ARGUMENTS:     *filename, the file from which to read in the array.    !
    !                       CHARACTER                                          !
    !                   r16array, the array which is to be filled with values  !
    !                       read in from the file. REAL(16)                    !
    !                   nrows, returns the number of rows of data which were   !
    !                       read into the array. INTEGER                       !
    !                   [errval], returns any error codes generated during the !
    !                       execution of this subroutine. INTEGER              !
    ...
    !   MODIFICATIONS:  None                                                   !
    !                                                                          !
    !   PURPOSE:        Writes out a 1 byte integer 2D array.                  !
    !                                                                          !
    !   ARGUMENTS:     *filename, the file to which to write in the array. If  !
    !                       it is 'stdout' then writes to terminal instead of  !
    !                       file. CHARACTER                                    !
    !                   i1array, the array which is to be written to the file. !
    !                       INTEGER(1)                                         !
    !                  *[clobber = 'clobber'], determines whether to overwrite !
    !                       existing files. Options are 'clobber' (overwrite   !
    !                       file if exists), 'noclobber' (produce error if     !
    ...
    !   MODIFICATIONS:  None                                                   !
    !                                                                          !
    !   PURPOSE:        Writes out an 8 byte integer 2D array.                 !
    !                                                                          !
    !   ARGUMENTS:     *filename, the file to which to write in the array. If  !
    !                       it is 'stdout' then writes to terminal instead of  !
    !                       file. CHARACTER                                    !
    ...
    !   MODIFICATIONS:  None                                                   !
    !                                                                          !
    !   PURPOSE:        Writes out a quad precision 2D array.                  !
    !                                                                          !
    !   ARGUMENTS:     *filename, the file to which to write in the array. If  !
    !                       it is 'stdout' then writes to terminal instead of  !
    !                       file. CHARACTER                                    !
    !                   r16array, the array which is to be written to the      !
    !                       file. REAL(16)                                     !
    !                  *[clobber = 'clobber'], determines whether to overwrite !
    !                       existing files. Options are 'clobber' (overwrite   !
    !                       file if exists), 'noclobber' (produce error if     !
